...
## North Stars
- macOS-only Electron app (React/Vite renderer) that manages shared MCP servers for Cursor, Claude Code, and Codex entirely offline.
...
- Distribution target: signed & hardened `.dmg` with `sandbox=true`, `contextIsolation=true`, `nodeIntegration=false`, strict CSP, and local assets only.
...
3. **Offline compliance** – ship all assets locally, disable analytics, forbid auto-update hooks, and test with the network disabled.
...
4. **Detection & Settings** – detector service for Cursor/Claude/Codex, settings cards showing status + paths, last sync timestamp, and disabled toggles for undetected apps.
...
6. **User Flow Wiring** – connect UI + tray actions to registry/Keychain/sync services, broadcast state updates, and provide `.bak` restore prompts for invalid configs.
